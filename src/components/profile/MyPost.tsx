...
import RecentPostModel from "../core/RecentPostModel";
...
function MyPost({ UserId }: { UserId: string }) {
  const [posts, setPosts] = useState<Post[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 3; // You can adjust this based on your needs

  useEffect(() => {
    fetch(`api/userpost/${UserId}?page=${currentPage}&pageSize=${pageSize}`)
  ...
  return (
    <div className="flex flex-col items-center gap-16 my-10 m-3">
      {posts.map((post) => {
        return (
          <RecentPostModel
